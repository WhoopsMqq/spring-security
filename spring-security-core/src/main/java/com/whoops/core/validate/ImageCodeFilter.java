...
import com.whoops.core.config.ValidateCodeController;
...
//OncePerRequestFilter:Spring提供的一个工具类保证过滤器每次只会被调用一次
public class ImageCodeFilter extends OncePerRequestFilter implements InitializingBean {
    ...
    public void afterPropertiesSet() throws ServletException {
        ...
        String[] configUrls = StringUtils.split(properties.getCode().getImage().getUrl());
    ...
    private void validate(ServletWebRequest request) throws ServletRequestBindingException,ValidateCodeException {
        String username = ServletRequestUtils.getRequiredStringParameter(request.getRequest(),"username");
        ImageCode imageCode = (ImageCode)sessionStrategy.getAttribute(request, ValidateCodeController.SESSION_IMAGE_KEY+username);
        ...
        if(!StringUtils.equals(imageCode.getResult()+"",codeInRequest)){
            sessionStrategy.removeAttribute(request,ValidateCodeController.SESSION_IMAGE_KEY);
